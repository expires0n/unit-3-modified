...
Item {
    ...
    property string currentCat: "fav"
    ...
    // Palette
    readonly property color paper:     "#14101D"
    readonly property color ink:       "#D8CDF5"
    readonly property color inkStrong: "#D8CDF5"
    readonly property color inkSoft:   "#8B7BC7"
    readonly property color lineSoft:  Qt.rgba(216/255,205/255,245/255,0.25)
    readonly property color lineVsoft: Qt.rgba(216/255,205/255,245/255,0.12)
    ...
    // BOOKMARKS sekmesine düşecek uygulamalar (küçük harfle yaz, ismin içinde aranır)
    readonly property var bookmarked: ["spotify", "org.vinegarhq.sober", "waterfox", "opencode", "discord", "steam", "curseforge", "drawboard-pdf", "telegram"]
    ...
    readonly property var catOrder: ["fav","all","dev","sys","net","media","office","graphics","games","other"]
    ...
    readonly property var filteredApps: {
        ...
        return apps.filter(function(a) {
            // arama yazınca kategori fark etmez, her yerde arar (boşken sekmeye uyar)
            var catOk = !!q || currentCat === "all" || a.cat === currentCat
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    var iconName  = (parts[4] || "").trim()
                    // Açık kaynak tema ikonu (yoksa generic), enrichment yok — hafif
                    var iconUrl   = iconName ? Quickshell.iconPath(iconName, "application-x-executable") : ""
                    if (!name || !desktopId) continue
                    // Utiliser l'exec brut si dispo, sinon le desktop ID
                    var launchCmd = rawExec || desktopId

                    var cat = "other"
                    var haystack = (name + " " + desktopId).toLowerCase()
                    var isFav = false
                    for (var b = 0; b < root.bookmarked.length; b++) {
                        if (haystack.indexOf(root.bookmarked[b]) >= 0) { isFav = true; break }
                    }
                    if (isFav) cat = "fav"
                    else if (/Development|IDE|TextEditor|Debugger/i.test(cats))          cat = "dev"
    ...
    Rectangle {
        anchors.fill: parent
        color: root.menuOpen ? Qt.rgba(20/255,16/255,29/255,0.6) : "transparent"
        visible: true
    ...
    Item {
        ...
        Rectangle {
            ...
            // Nokta dokusu + kuzey yıldızı (düşük opaklık)
            Canvas {
                id: dotTexture
                anchors.fill: parent
                onPaint: {
                    ...
                    var sx = w * 0.82, sy = h * 0.24
                    // noktalar (yıldız çevresi boş)
                    ctx.fillStyle = "rgba(216,205,245,0.13)"
                    for (var y = 12; y < h; y += 24) {
                        for (var x = 12; x < w; x += 24) {
                            var dx = x - sx, dy = y - sy
                            if (dx * dx + dy * dy < 75 * 75) continue
                            ctx.fillRect(x, y, 2, 2)
                        }
                    }
                    // kuzey yıldızı — dikey uzun + yatay kısa çizgi
                    ctx.strokeStyle = "rgba(167,139,250,0.30)"
                    ctx.lineWidth = 2
                    ctx.beginPath(); ctx.moveTo(sx, sy - 48); ctx.lineTo(sx, sy + 48); ctx.stroke()
                    ctx.beginPath(); ctx.moveTo(sx - 30, sy); ctx.lineTo(sx + 30, sy); ctx.stroke()
                    ctx.fillStyle = "rgba(216,205,245,0.45)"
                    ctx.beginPath(); ctx.arc(sx, sy, 3, 0, Math.PI * 2); ctx.fill()
                    // küçük yoldaş yıldız
                    var bx = w * 0.70, by = h * 0.40
                    ctx.strokeStyle = "rgba(167,139,250,0.22)"
                    ctx.lineWidth = 1.5
                    ctx.beginPath(); ctx.moveTo(bx, by - 20); ctx.lineTo(bx, by + 20); ctx.stroke()
                    ctx.beginPath(); ctx.moveTo(bx - 12, by); ctx.lineTo(bx + 12, by); ctx.stroke()
                    ctx.fillStyle = "rgba(216,205,245,0.35)"
                    ...
                    Column {
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                Rectangle {
                                    anchors.fill:parent
                                    color: parent.isActive ? root.ink : (catMA.containsMouse ? Qt.rgba(216/255,205/255,245/255,0.07) : "transparent")
                                ...
                                Text {
                                    ...
                                    color: parent.isActive ? Qt.rgba(36/255,27/255,56/255,0.6) : Qt.rgba(139/255,123/255,199/255,0.5)
                                }
                            ...
                            delegate: Item {
                                ...
                                Row {
                                    ...
                                    Text {
                                        ...
                                        color: appMA.containsMouse||appDelegate.isFocused ? Qt.rgba(36/255,27/255,56/255,0.5) : root.inkSoft
                                    ...
                                    Item {
                                        ...
                                        Text {
                                            ...
                                            color: appMA.containsMouse||appDelegate.isFocused ? Qt.rgba(36/255,27/255,56/255,0.9) : root.ink
                                            Behavior on color { ColorAnimation { duration:120 } }
                                        }
                                        IconImage {
                                            id: iconImg
                                    ...
                                    Column {
                                        ...
                                        Text {
                                            ...
                                            color: appMA.containsMouse||appDelegate.isFocused ? Qt.rgba(36/255,27/255,56/255,1) : root.ink
                                            Behavior on color { ColorAnimation { duration:120 } }
                                        }
                                        Text {
                                    ...
                                    Text {
                                        ...
                                        color: appMA.containsMouse||appDelegate.isFocused ? Qt.rgba(36/255,27/255,56/255,0.4) : root.inkSoft
                                        Behavior on color { ColorAnimation { duration:120 } }
                                    }
                                    Text {
                                        anchors.verticalCenter:parent.verticalCenter
        ...
        Rectangle {
            ...
            color:"#A78BFA"; z:50; width:2; x:root.lw-2
        }
    }
